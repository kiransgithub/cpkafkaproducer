...
timestamp_endtime_inclusive_YYYYMMDDTHHMMSS_zulu = time.strftime("%Y-%m-%dT%H:%M:%SZ",time.gmtime(timestamp_endtime_inclusive / 1000))
...
if checkPointMethod == 'TimeBased':
    ...
    output_filename = output_filename_prefix + "_" + startPoint_time_YYYYMMDDTHHMMSS_zulu + "_" + timestamp_endtime_inclusive_YYYYMMDDTHHMMSS_zulu + "_" + time.strftime("%Y%m%d%H%M%SZ",time.gmtime(now_time_float)) + ".json"
elif checkPointMethod == 'SequenceBased':
    output_filename = output_filename_prefix + "_" + str(sequenceBased_column) + "_" + str(startingpoint) + "_" + time.strftime("%Y%m%d%H%M%SZ",time.gmtime(now_time_float)) + ".json"
