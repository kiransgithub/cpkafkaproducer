...
import pyodbc
import json
from kafkaproducer_old import KafkaProducer
import asyncio
...
now_time_YYYYMMDDTHHMMSS_zulu = time.strftime("%Y%m%d%H%M%SZ", time.gmtime(now_time_float))
print("now_time_YYYYMMDDTHHMMSS_zulu value:" + now_time_YYYYMMDDTHHMMSS_zulu)

timestamp_endtime_inclusive_YYYYMMDDTHHMMSS_zulu = time.strftime("%Y%m%d%H%M%SZ",
...
startPoint_time_YYYYMMDDTHHMMSS_zulu = time.strftime("%Y%m%d%H%M%SZ", time.gmtime(TimeInsecs))
output_filename = output_filename_prefix + "_" + startPoint_time_YYYYMMDDTHHMMSS_zulu + "_TO_" + timestamp_endtime_inclusive_YYYYMMDDTHHMMSS_zulu + ".json"
...
print("Using StartPoint: " + str(startingpoint))
...
kafka_producer = KafkaProducer('kafka_config.yaml')
...
while rows:
    ...
    for row in rows:
        ...
        batch_messages.append(json.dumps(row_dictionary_datastruct, default=str))
    ...
    asyncio.run(kafka_producer.produce_messages(batch_messages))
...
# Flush Kafka messages and handle any failed messages
kafka_producer.flush()
failed_messages = kafka_producer.get_failed_messages()
if failed_messages:
    kafka_producer.write_failed_messages('failed_messages.json')