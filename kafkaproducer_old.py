...
class KafkaProducer:
    ...
    def delivery_report(self, err, msg):
        ...
        if err is not None:
            ...
            with self.failed_messages_lock:
                ...
                self.failed_messages.append(json.loads(msg.value()))
        else:
    ...
    async def produce_message(self, message: str, headers: List[Tuple[str, str]]):
        ...
        """
        try:
            self.producer.produce(self.topic, value=message, headers=headers, callback=self.delivery_report)
            ...
            with self.failed_messages_lock:
                self.logger.info(f'Appending to failed_message list for retrying...')
                self.failed_messages.append((json.loads(message), headers))
